...
get it really easy to write tests, so we tried to make simple tests
...
implementation details and ideally should be kept out of a .h. So often I get
...
Currently, the following `TEST`, `FAIL`, `SUCCEED`, and the BasicArgKey comparison assertion macros can have alternative names. You can see the full list of covered macros [here](http://www.google.com/codesearch?q=if+!GTEST_DONT_DEFINE_\w%2B+package:http://googletest\.googlecode\.com+file:/include/gtest/gtest.h). More information can be found in the "Avoiding Macro Name Clashes" section of the README file.
...
## How do I build_ Google Testing Framework with Xcode 4? ##

If you try to build_ Google Test's Xcode project with Xcode 4.0 or later, you may encounter an error message that looks like
